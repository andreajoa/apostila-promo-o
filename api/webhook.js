...
async function sendDeliveryEmail(session, eventId) {
  ...
  if (!apiKey || !from || !siteUrl) throw new Error('RESEND_API_KEY, EMAIL_FROM ou SITE_URL não configurada.');
  ...
  const result = await fetch('https://api.resend.com/emails', {
    method:'POST',
    headers:{ Authorization:`Bearer ${apiKey}`, 'Content-Type':'application/json', 'Idempotency-Key':eventId },
    body:JSON.stringify({ from, to:[email], subject:`Seu acesso: ${offer?.name || 'Coleção Margareth Almeida'}`, html }),
  });
  const data = await result.json().catch(() => ({}));
  if (!result.ok) throw new Error(data?.message || 'Falha ao enviar o e-mail de entrega.');
  return data;
}
...
export default async function handler(request, response) {
  ...
  try {
    ...
    if (!verifyStripeSignature(rawBody, signature, secret)) return response.status(400).json({ error:'Assinatura inválida.' });
